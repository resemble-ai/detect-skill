...
Give any AI agent the ability to detect AI-generated audio, images, video, and text, then analyze completed detection results using Resemble's Detect and Intelligence APIs.
...
The result carries `prediction` (`ai` or `human`) and `confidence` (how sure the model is of that prediction). Text under 25 words is rejected rather than guessed.
...
- **Decision tree** — maps user intent to Detect, Intelligence, Detect Intelligence, Detect Agents, or Text Detection endpoints
- **Direct API examples** — curl-first workflows for URL, file upload, secure upload, and polling
- **Score interpretation** — how to read and present media scores and text `prediction` + `confidence` pairs
